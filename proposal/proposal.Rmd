...
ggplot(data = yt, mapping = aes(x = month(publish_time, label = TRUE), y = views)) +
  geom_point(size = 0.5) + 
  coord_polar() + 
  labs(title = "Views and Upload Month", x="Month", y="Views") + 
  scale_y_continuous(labels = label_number(suffix = " M", scale = 1e-6))
...
ggplot(data = mean_views_for_category, mapping = aes(x = reorder(category, mean_views), y = mean_views)) +
 ...
 theme(legend.position= 'none') +
  coord_flip() + 
  scale_y_continuous(labels = label_number(suffix = " M", scale = 1e-6)) +
  labs( "Mean Views For Each Category", x="Mean Views", y="Category")
```



```{r cateogry-v-views-median, echo = FALSE, message = FALSE}
...
ggplot(data = median_views_for_category, mapping = aes(x = reorder(category, median_views), y = median_views)) +
 ...
 theme(legend.position= 'none') +
  coord_flip() +
  scale_y_continuous(labels = label_number(suffix = " M", scale = 1e-6)) +
  labs( "Median Views For Each Category", x="Median Views", y="Category")
```
