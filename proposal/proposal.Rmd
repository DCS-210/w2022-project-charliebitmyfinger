...
```{r data-visual}
ggplot(data = yt, mapping = aes(x = views, y = likes), alpha = 0.1) +
  geom_point(size = 0.5) +
...
```{r length-of-title-density-version}
ggplot(data = yt, mapping = aes(x = title_length, group = category, fill = category)) +
...
#IM NOT SURE IF I SET UP THE Y (VIEWS) CORRECTLY FOR THIS ONE
```
